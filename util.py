...
def post_listing_to_slack(sc, listing, slack_channel):
    """
    ...
    if listing["price"] is None:
        listing["price"] = "Free"
    print(listing["url"].find('zip',0,len(listing["url"])))
    if listing["url"].find('zip',0,len(listing["url"])) != -1:
        desc = "{0}  |  {1}  |  {2}  |  <{3}>".format(listing["name"], listing["where"], listing["price"], listing["url"])
        sc.api_call(
            "chat.postMessage", json={'channel':'#craigslist', 'text':desc,
